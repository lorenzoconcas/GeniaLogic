...
export function generationLayout(people: Person[], relationships: Relationship[]) {
  ...
  const validLinks = relationships.filter(
    (link) => groupOf.has(link.sourceId) && groupOf.has(link.targetId),
  )
  ...
  const join = (first: string, second: string) => {
    ...
    for (const [id, group] of groupOf)
      if (group === a || group === b) groupOf.set(id, representative)
  }
  ...
  for (const childId of ids) {
    const parents = [
      ...new Set(
        parentLinks.filter((link) => link.targetId === childId).map((link) => link.sourceId),
      ),
    ].sort()
  ...
  const rank = (id: string): number => {
    if (!ranks.has(id))
      ranks.set(id, Math.max(0, ...[...parents.get(id)!].map((parent) => rank(parent) + 1)))
    return ranks.get(id)!
  }
  ...
  const width = (id: string): number => {
    if (!widths.has(id)) {
      ...
      widths.set(
        id,
        Math.max(
          members.get(id)!.length * cardPitch,
          descendants.reduce((sum, child) => sum + width(child), 0) +
            Math.max(0, descendants.length - 1) * branchGap,
        ),
      )
    }
