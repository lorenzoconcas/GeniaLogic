...
export async function saveLocal(tree: FamilyTree): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    transaction.onabort = () =>
      reject(transaction.error ?? new Error('Salvataggio locale interrotto.'))
  })
  db.close()
}
...
async function decodeTree(file: File): Promise<FamilyTree> {
  ...
  try {
    ...
    throw new Error(
      'Non riesco a leggere il file. Se è nel cloud, scaricalo sul dispositivo e riprova.',
    )
  }
  ...
  if (typeof DecompressionStream === 'undefined') {
    throw new Error(
      'Questo browser non supporta l’apertura degli archivi compressi. Aggiorna il browser o il sistema operativo e riprova.',
    )
  }
  ...
  try {
    ...
    throw new Error('Impossibile leggere i dati del file.')
  }
  if (!isFamilyTree(parsed))
    throw new Error('La struttura del file non è compatibile con GeniaLogic.')
  return parsed
}
...
function isFamilyTree(value: unknown): value is FamilyTree {
  ...
  if (
    !(
      ...
      Array.isArray(tree.relationships) &&
      tree.people.every(
        (p) =>
          p &&
          typeof p.id === 'string' &&
      ...
      tree.relationships.every(
        (r) =>
          r &&
          typeof r.id === 'string' &&
          typeof r.sourceId === 'string' &&
          typeof r.targetId === 'string' &&
          relationshipTypes.has(r.type),
      )
    )
  )
    return false
  ...
  return (
    personIds.size === tree.people.length &&
    tree.relationships.every(
      (relationship) =>
        personIds.has(relationship.sourceId) && personIds.has(relationship.targetId),
    )
  )
}
...
export async function exportTree(tree: FamilyTree): Promise<'picker' | 'download'> {
  ...
  if (picker) {
    const handle = await picker({
      ...
      types: [
        {
          description: 'Archivio GeniaLogic',
          accept: { 'application/x-genia-family-tree': ['.genia'] },
        },
      ],
    })
...
export async function pickTreeHandle(): Promise<GeniaFileHandle | null> {
  const picker = (
    window as Window & { showOpenFilePicker?: (options: unknown) => Promise<GeniaFileHandle[]> }
