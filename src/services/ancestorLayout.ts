...
export const ancestorColor = (person: Person) =>
  /^#[\da-f]{6}$/i.test(person.color) ? person.color : '#5657d9'
...
export function ancestorLayout(
  ...
  if (!Number.isInteger(generations) || generations < 1 || generations > 10)
    throw new Error('Scegli da 1 a 10 generazioni.')
  ...
  for (const relationship of relationships) {
    if (
      !Object.hasOwn(ancestorLinkStyles, relationship.type) ||
      !byId.has(relationship.sourceId) ||
      !byId.has(relationship.targetId)
    )
      continue
    const list = parents.get(relationship.targetId) ?? []
    if (
      !list.some(
        (item) => item.sourceId === relationship.sourceId && item.type === relationship.type,
      )
    )
  ...
  for (const list of parents.values())
    list.sort((a, b) => {
      const first = byId.get(a.sourceId)!,
        second = byId.get(b.sourceId)!
      const rank = (p: Person) => (p.gender === 'male' ? 0 : p.gender === 'female' ? 1 : 2)
      return (
        rank(first) - rank(second) ||
        first.lastName.localeCompare(second.lastName, 'it') ||
        first.firstName.localeCompare(second.firstName, 'it') ||
  ...
  function visit(
    ...
    const ancestors =
      !cycle && generation < generations
        ? (parents.get(personId) ?? []).map((link) =>
            visit(link.sourceId, generation + 1, nextPath, node.id, link.type),
          )
        : []
    children.set(node.id, ancestors)
    node.span = Math.max(
      1,
      ancestors.reduce((sum, item) => sum + item.span, 0),
    )
    return node
  }
  ...
  function place(node: AncestorNode, start: number) {
    ...
    let offset = start
    for (const ancestor of children.get(node.id) ?? []) {
      place(ancestor, offset)
      offset += ancestor.span
    }
  }
  place(root, 0)
  ...
  const links = nodes
    ...
    .map((node) => {
      ...
      let path: string
      if (orientation === 'horizontal') {
        const x = child.x + cardWidth,
          y = child.y + cardHeight / 2,
          targetY = node.y + cardHeight / 2
        const middle = (x + node.x) / 2
        path = `M ${x} ${y} C ${middle} ${y} ${middle} ${targetY} ${node.x} ${targetY}`
      } else {
        const x = child.x + cardWidth / 2,
          y = child.y,
          targetX = node.x + cardWidth / 2,
          targetY = node.y + cardHeight
  ...
  return {
    ...
    generations,
    orientation,
    cycles: nodes.some((node) => node.cycle),
  }
}
...
export function ancestorSvg(chart: AncestorChart): string {
  const parts = [
    `<text x="40" y="42" font-size="26" font-weight="700">Albero degli antenati</text>`,
    `<text x="40" y="74" font-size="16">GeniaLogic · ${chart.generations} generazioni · ${chart.orientation === 'horizontal' ? 'Orizzontale' : 'Verticale'}</text>`,
  ]
  for (const link of chart.links)
    parts.push(
      `<path d="${link.path}" fill="none" stroke="${link.color}" stroke-width="2" stroke-dasharray="${link.dash}"><title>${link.label}</title></path>`,
    )
  for (const node of chart.nodes) {
    parts.push(
      `<g><title>${escapeXml(`${node.person.firstName} ${node.person.lastName}`)}</title><rect x="${node.x}" y="${node.y}" width="${chart.cardWidth}" height="${chart.cardHeight}" rx="12" fill="${node.generation ? '#fff' : '#eeefff'}" stroke="${ancestorColor(node.person)}" stroke-width="${node.generation ? 2 : 4}"/>`,
    )
    parts.push(
      `<text text-anchor="middle" font-size="18" font-weight="600">${node.lines.map((line, index) => `<tspan x="${node.x + chart.cardWidth / 2}" y="${node.y + (chart.cardHeight - node.lines.length * 24) / 2 + 19 + index * 24}">${escapeXml(line)}</tspan>`).join('')}</text></g>`,
    )
  }
  Object.values(ancestorLinkStyles).forEach((style, index) =>
    parts.push(
      `<path d="M ${40 + index * 190} ${chart.height - 27} h 32" stroke="${style.color}" stroke-width="2" stroke-dasharray="${style.dash}"/><text x="${82 + index * 190}" y="${chart.height - 22}" font-size="16">${style.label}</text>`,
    ),
  )
