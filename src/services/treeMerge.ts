...
function mergeCollection<T extends { id: string }>(
  ...
  for (const id of ids) {
    ...
    if (!original) {
      if (ours && theirs)
        result.push(same(ours, theirs) ? ours : mergeRecord({ id } as T, ours, theirs, conflict))
...
export function mergeTrees(
  ...
  const relationships = mergeCollection<Relationship>(
    base.relationships,
    local.relationships,
    remote.relationships,
    conflict,
  ).filter(
    (relationship) => personIds.has(relationship.sourceId) && personIds.has(relationship.targetId),
  )
