...
const color = (person?: Person) =>
  /^#[\da-f]{6}$/i.test(person?.color ?? '') ? person!.color : '#6264c9'
...
function wrap(text: string, maxWidth: number, measure: MeasureText): string[] {
  ...
  for (const word of text.trim().split(/\s+/u)) {
    ...
    for (const char of word) {
      if (line && measure(line + char, fontSize) > maxWidth) {
        lines.push(line)
        line = ''
      }
      line += char
...
function personLabel(person: Person, measure: MeasureText) {
  ...
  return {
    lines,
    width: Math.max(...lines.map((line) => measure(line, fontSize))) * 1.15 + 8,
    height: lines.length * lineHeight,
  }
}

export function createFanExport(
  root: Person,
  slots: ExportSlot[],
  generations: number,
  ...
  if (!Number.isInteger(generations) || generations < 1 || generations > 10)
    throw new Error('Numero di generazioni non valido.')
  ...
  for (let generation = 1; generation <= generations; generation++) {
    ...
    const members = slots
      ...
      .map((slot) => ({
        ...slot,
        label: slot.person ? personLabel(slot.person, measure) : undefined,
      }))
  ...
  const parts: string[] = []
  const textLines = (lines: string[], x: number, y: number, fill = '#24283c') =>
    `<text x="${x}" y="${y}" text-anchor="middle" fill="${fill}" font-size="${fontSize}" font-weight="600">${lines.map((line, index) => `<tspan x="${x}" dy="${index ? lineHeight : 0}">${escapeXml(line)}</tspan>`).join('')}</text>`
  const polar = (r: number, a: number) => `${cx + Math.cos(a) * r} ${cy - Math.sin(a) * r}`
  parts.push(
    `<text x="40" y="42" font-size="26" font-weight="700" fill="#24283c">${shape === 'circle' ? 'Albero genealogico radiale' : 'Ventaglio genealogico'}</text>`,
  )
  parts.push(
    `<text x="40" y="72" font-size="16" fill="#555c70">GeniaLogic · ${generations} generazioni di antenati</text>`,
  )
  for (const ring of rings) {
    for (const slot of ring.members) {
      ...
      parts.push(
        `<path d="${path}" fill="${slot.person ? color(slot.person) : '#f1f3f8'}" fill-opacity="${slot.person ? '.14' : '1'}" stroke="#b9c0d1" stroke-width="1"/>`,
      )
      if (!slot.person || !slot.label) continue
      const rotation = readableRotation((-middle * 180) / Math.PI)
      parts.push(
        `<g transform="translate(${polar(ring.labelRadius, middle)}) rotate(${rotation})"><title>${escapeXml(fullName(slot.person))}</title>${textLines(slot.label.lines, 0, -slot.label.height / 2 + 19)}</g>`,
      )
    }
  }
  parts.push(
    `<circle cx="${cx}" cy="${cy}" r="${rootRadius}" fill="#eeefff" stroke="${color(root)}" stroke-width="3"/>`,
  )
  ...
  const groupBottoms = groups.map((group, index) => {
    ...
    let y = footerY
    parts.push(
      `<text x="${x}" y="${y}" font-size="22" font-weight="700" fill="#24283c">${escapeXml(group.title)} (${group.people.length})</text>`,
    )
    y += 35
    if (!group.people.length) {
      parts.push(
        `<text x="${x}" y="${y}" font-size="18" fill="#555c70">Nessuna persona registrata</text>`,
      )
      y += 28
    }
    for (const person of group.people) {
      ...
      parts.push(
        `<rect x="${x}" y="${y - 20}" width="${columnWidth}" height="${height}" rx="8" fill="#f5f6fb"/>`,
      )
...
export function pngScale(width: number, height: number) {
  ...
  if (scale < 1)
    throw new Error(
      'Questo grafico è troppo grande per un PNG leggibile. Esportalo in SVG oppure riduci le generazioni.',
    )
  return scale
}
    ...
    return await new Promise<Blob>((resolve, reject) =>
      canvas.toBlob(
        (blob) =>
          blob
            ? resolve(blob)
            : reject(new Error('Impossibile creare il PNG. Prova il formato SVG.')),
        'image/png',
      ),
    )
  } finally {
