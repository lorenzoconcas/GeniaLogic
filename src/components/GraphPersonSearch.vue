  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <span
            class="graph-search-avatar"
            :style="{ background: person.color }"
            aria-hidden="true"
          >
            {{ person.firstName[0] }}{{ person.lastName[0] }}
          </span>
          <span class="graph-search-identity">
            <strong>{{ person.firstName }} {{ person.lastName }}</strong>
            <small>{{ personSearchDetails(person) }}</small>
            <small
              v-if="namesakes.has(person.id)"
              class="graph-search-reference"
            >
              Rif. {{ person.id }}
            </small>
          </span>
...
.graph-search-field:focus-within {
...
.graph-search-field input::-webkit-search-cancel-button {
  appearance: none;
}
.graph-search-clear {
  display: grid;
...
.graph-search-dropdown {
  ...
  background: #fff;
  box-shadow: 0 12px 35px #24283c26;
  overflow: hidden;
}
.graph-search-count {
  margin: 0;
...
.graph-search-result[aria-pressed='true'] {
  background: #f0f0ff;
}
...
.graph-search-identity strong {
  font-size: 1rem;
  line-height: 1.35;
  color: #24283c;
}
.graph-search-identity small {
  font-size: 0.875rem;
  line-height: 1.4;
  color: #596176;
}
.graph-search-identity .graph-search-reference {
  color: #4f50ad;
  font-size: 0.8125rem;
}
.graph-search-empty {
