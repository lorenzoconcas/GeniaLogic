...
import { ancestorColor, ancestorLayout, ancestorSvg, type AncestorOrientation } from '../services/ancestorLayout'
...
async function zoomBy(factor: number) {
  ...
  zoom.value = Math.min(2, Math.max(.02, old * factor))
  await nextTick()
  ...
  <section class="ancestor-tree" aria-label="Vista degli antenati">
    <div class="ancestor-toolbar">
      ...
      <div class="ancestor-settings">
        ...
        <label>Generazioni<select v-model.number="generations"><option v-for="n in 6" :key="n" :value="n">{{ n }}</option></select></label>
    ...
    <div ref="viewport" class="ancestor-viewport" tabindex="0" aria-label="Grafico degli antenati: usa lo zoom e scorri per esplorare">
      ...
      <svg v-if="result.chart" :width="result.chart.width * zoom" :height="result.chart.height * zoom" :viewBox="`0 0 ${result.chart.width} ${result.chart.height}`" font-family="Arial, sans-serif" aria-label="Albero degli antenati">
        ...
        <g v-for="node in result.chart.nodes" :key="node.id" class="ancestor-person" role="button" tabindex="0" :aria-label="`Mostra antenati di ${node.person.firstName} ${node.person.lastName}`" @click="emit('selectPerson', node.person.id)" @keydown.enter.prevent="emit('selectPerson', node.person.id)" @keydown.space.prevent="emit('selectPerson', node.person.id)">
...
.ancestor-person:hover rect,.ancestor-person:focus-visible rect { stroke:#3637b8; stroke-width:5; fill:#ededff; }
